function [tenS_new, all_U_new, tenX_hat_new, nmse_new, n_iteratons] = hooi(tenX, varargin)
% varargin is the n-rank used given by the user. If no n-rank is given,
% then hooi compute the rank of the n-mode folding of tenX and use it as
% the n-rank
% HOSVD
if isempty(varargin) % no n-rank is given
    [tenS_cur, all_U_cur, ~] = thosdv(tenX);
else
    [tenS_cur, all_U_cur, ~] = thosdv(tenX, varargin{1});
end
...
function [tenS, varout, multilinear_ranks] = thosdv(tenX, varargin)
% varargin is the n-rank used given by the user. If no n-rank is given,
