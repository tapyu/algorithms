...
# timerun generation ---------------------------------------------------------
max_=100
for (n in xdata <- c(2, 4, 8, 16, 32, 64)){
# matrices
...
plot(xdata, delta_t_met1, log="y", xlab="N", ylab="Time (seconds)", col="blue", pch="x", type="o", lty="dashed", lwd = 2, main="Runtime for the Kronecker Product computation")
lines(xdata, delta_t_met2, col="red", pch="o", type="o", lty="dashed", lwd = 2)
legend(0 , 110, legend=c(TeX("$(A\\oplus B)^{-1}$"), TeX("$A^{-1}\\oplus B^{-1}")), col=c("blue","red"), pch=c("x","o"), lty=c("dashed", "dashed"))